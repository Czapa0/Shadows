	...
	m_particles{ {-1.3f, -0.6f, -0.14f} }
{
	...
	XMMATRIX lightProjMtx = XMMatrixPerspectiveFovLH(LIGHT_FOV_ANGLE, 1.0f, LIGHT_NEAR, LIGHT_FAR);
	XMStoreFloat4x4(&m_lightProjMtx, lightProjMtx);
	//Sampler States
...
void RoomDemo::UpdateLamp(float dt)
{
	...
	XMStoreFloat4(&lightPos, XMVector3TransformCoord(pos, lamp));
	XMStoreFloat4(&lightTarget, XMVector3TransformCoord(target, lamp));
	UpdateBuffer(m_cbLightPos, lightPos);

	// TODO : 1.01 Calculate light's view and inverted view matrix
	XMMATRIX lightViewMtx = XMMatrixLookAtLH(XMLoadFloat4(&lightPos), XMLoadFloat4(&lightTarget), XMLoadFloat4(&upDir));
	XMMATRIX invLightViewMtx = XMMatrixInverse(0, lightViewMtx);
	XMStoreFloat4x4(&m_lightViewMtx[0], lightViewMtx);
	XMStoreFloat4x4(&m_lightViewMtx[1], invLightViewMtx);

	// TODO : 1.03 Calculate map transform matrix
	XMMATRIX lightProjMtx = XMLoadFloat4x4(&m_lightProjMtx);
	XMMATRIX lightMtx = lightViewMtx * lightProjMtx * XMMatrixScaling(0.5f, -0.5f, 1.0f) * XMMatrixTranslation(0.5f, 0.5f, 0.0f);

	// TODO : 1.19 Modify map transform to fix z-fighting
	lightMtx = lightMtx * XMMatrixTranslation(0.0f, 0.0f, -0.00001f);

	XMStoreFloat4x4(&texMtx, lightMtx);
