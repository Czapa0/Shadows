...
float4 main(PSInput i) : SV_TARGET
{
    ...
    float4 texCoords = mul(mapMtx, float4(i.worldPos, 1.0f));
    ...
    lightColor = max(lightColor, lightMap.Sample(colorSampler, texCoords.xy));
    ...
    float4 shadowColor = shadowMap.Sample(colorSampler, texCoords.xy);
    if (shadowColor.r < texCoords.z)
    {
        lightColor = defLightColor;
    }
